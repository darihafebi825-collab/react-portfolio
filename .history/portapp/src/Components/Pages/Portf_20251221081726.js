...
import { Briefcase, Code2, Calendar, MapPin, ChevronRight } from "lucide-react";
...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-24 bg-gradient-to-b from-gray-100 to-gray-200 relative overflow-hidden">

      {/* 3D Grid background with pure CSS gradients */}
      <div className="absolute inset-0" style={{
        backgroundImage: `
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-20">
          <motion.div initial={{ scale: 0, rotateY: -180 }} whileInView={{ scale: 1, rotateY: 0 }} transition={{ type: "spring", stiffness: 200, damping: 15 }} className="inline-block mb-6">
            <div className="relative" style={{ transform: 'perspective(1000px) rotateY(10deg)', transformStyle: 'preserve-3d' }}>
              <div className="absolute inset-0 bg-gray-900 rounded-2xl blur-xl opacity-50" style={{ transform: 'translateZ(-20px)' }} />
              <div className="relative bg-gradient-to-br from-gray-900 to-gray-700 p-6 rounded-2xl shadow-2xl">
                <Briefcase size={48} className="text-white" />
              </div>
            </div>
          </motion.div>

          <h2 className="text-5xl md:text-6xl font-bold text-gray-900 mb-4">Experience Showcase</h2>
          <motion.div initial={{ width: 0 }} whileInView={{ width: 100 }} transition={{ duration: 0.8, delay: 0.3 }} className="h-1.5 bg-gray-900 mx-auto mb-4 rounded-full" />
          <p className="text-gray-700 max-w-2xl mx-auto text-lg font-medium">A 3D journey through my professional experience</p>
        </motion.div>

        <div className="relative" style={{ perspective: '2000px', perspectiveOrigin: 'center top' }}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8" style={{ transformStyle: 'preserve-3d' }}>
            {experiences.map((exp, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 100, rotateX: -45 }} whileInView={{ opacity: 1, y: 0, rotateX: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.2, duration: 0.8, type: "spring", stiffness: 100 }} className="relative" style={{ transformStyle: 'preserve-3d', transform: `translateZ(${idx * 30}px) rotateY(${idx % 2 === 0 ? '5' : '-5'}deg)` }}>
                <motion.div whileHover={{ y: -20, rotateX: -5, transition: { duration: 0.3 } }} className="relative" style={{ transformStyle: 'preserve-3d' }}>

                  {/* Shadow under platform */}
                  <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 w-4/5 h-8 bg-black/30 rounded-full blur-2xl" style={{ transform: 'translateZ(-80px) scale(1.2)' }} />

                  {/* Platform base */}
                  <div className="absolute -bottom-6 left-0 right-0 h-3 bg-gradient-to-b from-gray-400 to-gray-500 rounded-t-lg" style={{ transform: 'translateZ(-40px) rotateX(90deg)', transformOrigin: 'top' }} />
                  <div className="absolute -bottom-6 left-0 w-3 h-12 bg-gradient-to-r from-gray-500 to-gray-600" style={{ transform: 'translateZ(-40px) rotateY(-90deg)', transformOrigin: 'right' }} />
                  <div className="absolute -bottom-6 right-0 w-3 h-12 bg-gradient-to-l from-gray-500 to-gray-600" style={{ transform: 'translateZ(-40px) rotateY(90deg)', transformOrigin: 'left' }} />

                  {/* Main card */}
                  <div className="relative bg-white rounded-xl p-6 shadow-2xl border-2 border-gray-200 overflow-hidden group" style={{ transformStyle: 'preserve-3d', boxShadow: '0 30px 60px rgba(0,0,0,0.3), 0 0 0 1px rgba(0,0,0,0.1)' }}>
                    <div className={`absolute top-0 left-0 right-0 h-2 bg-gradient-to-r ${exp.color}`} />
                    <motion.div initial={{ scale: 0, rotateY: -180 }} whileInView={{ scale: 1, rotateY: 0 }} transition={{ delay: idx * 0.2 + 0.3, type: "spring" }} className={`absolute -top-4 -right-4 w-16 h-16 bg-gradient-to-br ${exp.color} rounded-full flex items-center justify-center font-bold text-2xl text-white shadow-2xl border-4 border-white`} style={{ transformStyle: 'preserve-3d', transform: 'translateZ(30px)' }}>{idx + 1}</motion.div>

                    <div className="relative z-10 pt-2">
                      <div className="flex items-start gap-3 mb-4">
                        <motion.div whileHover={{ rotateY: 360 }} transition={{ duration: 0.6 }} className={`bg-gradient-to-br ${exp.color} p-3 rounded-xl shadow-lg`} style={{ transformStyle: 'preserve-3d', transform: 'translateZ(20px)' }}>
                          <Code2 size={24} className="text-white" />
                        </motion.div>
                        <div className="flex-1 min-w-0">
                          <h3 className="text-xl font-bold text-gray-900 mb-1 leading-tight">{exp.title}</h3>
                          <p className="text-base text-gray-700 font-semibold">{exp.company}</p>
                        </div>
                      </div>

                      <div className="flex flex-wrap gap-2 mb-4">
                        <span className="flex items-center gap-1.5 bg-gray-100 px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-200"><Calendar size={12} />{exp.period}</span>
                        <span className={`flex items-center gap-1.5 bg-gradient-to-r ${exp.color} text-white px-3 py-1.5 rounded-lg text-xs font-medium shadow-md`}><MapPin size={12} />{exp.location}</span>
                      </div>

                      <p className="text-sm text-gray-700 mb-3 pl-3 border-l-2 border-gray-900 italic leading-relaxed">{exp.description}</p>

                      {exp.responsibilities && (
                        <ul className="space-y-1.5 mb-4">
                          {exp.responsibilities.map((resp, i) => (
                            <motion.li key={i} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.2 + 0.1 * i }} className="text-xs text-gray-700 flex items-start gap-2">
                              <ChevronRight size={14} className="mt-0.5 flex-shrink-0 text-gray-900" />
                              <span>{resp}</span>
                            </motion.li>
                          ))}
                        </ul>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.tech.slice(0,5).map((t,i)=>(
                          <motion.span key={i} initial={{ opacity:0, scale:0 }} whileInView={{opacity:1, scale:1}} transition={{ delay: idx*0.2 + 0.05*i, type:"spring"}} whileHover={{scale:1.1, y:-3, transition:{duration:0.2}}} className="px-2.5 py-1 bg-gray-900 text-white text-xs rounded-lg font-medium shadow-md hover:shadow-xl transition-shadow" style={{transformStyle:'preserve-3d'}}>{t}</motion.span>
                        ))}
                        {exp.tech.length>5 && <span className="px-2.5 py-1 bg-gray-200 text-gray-700 text-xs rounded-lg font-medium">+{exp.tech.length-5}</span>}
                      </div>
                    </div>

                    <div className={`absolute top-0 right-0 w-2 h-full bg-gradient-to-b ${exp.color} opacity-20`} style={{transform:'translateZ(-5px) rotateY(90deg)', transformOrigin:'left'}} />
                    <div className={`absolute bottom-0 left-0 right-0 h-2 bg-gradient-to-r ${exp.color} opacity-20`} style={{transform:'translateZ(-5px) rotateX(90deg)', transformOrigin:'top'}} />
                  </div>
                </motion.div>
              </motion.div>
            ))}
