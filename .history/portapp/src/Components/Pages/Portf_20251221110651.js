...
const Projects = () => {
  ...
  const projects = [
    {
      ...
      description: 'Deep learning system that detects and classifies yoga poses from images or video using computer vision techniques.',
      image: '🧘‍♂️',
      tech: ['Python', 'TensorFlow', 'Keras', 'OpenCV'],
      features: ['Yoga Pose Classification', 'Real-time Detection', 'Pose Accuracy Evaluation']
    },
    {
      title: 'Water Potability Prediction',
      category: 'Machine Learning / Data Science',
      description: 'ML system that predicts water potability by analyzing physicochemical parameters for safe drinking water assessment.',
      image: '💧',
      tech: ['Python', 'Scikit-learn', 'Pandas', 'NumPy'],
      features: ['Water Quality Classification', 'Feature Engineering', 'Potability Prediction']
    },
    {
      title: 'Restaurant Website',
      category: 'Frontend Web Development',
      description: 'Responsive restaurant website showcasing menu, chef specials, and contact details with interactive UI.',
      image: '🍽️',
      tech: ['HTML', 'CSS', 'JavaScript'],
      features: ['Responsive Design', 'Interactive Menu', 'Table Reservation']
    },
    {
      title: 'Code Debugger',
      category: 'Developer Tool / Web App',
      description: 'Web-based debugging tool for Python and JavaScript with real-time error detection and output visualization.',
      image: '🐞',
      tech: ['React', 'JavaScript', 'Python', 'Node.js'],
      features: ['Code Execution', 'Error Detection', 'Debug Output']
    },
    {
      title: 'Money Manager Backend',
      category: 'Backend Development',
      description: 'Backend system built with Django to manage personal finance data, including income, expenses, and budgeting.',
      image: '💼',
      tech: ['Python', 'Django', 'PostgreSQL'],
      features: ['User Authentication', 'Expense Management', 'Budget Tracking']
    },
    {
      title: 'Phishing Link Detection',
      category: 'Machine Learning / Cybersecurity',
      description: 'ML system that detects phishing URLs by analyzing lexical and host-based features to prevent online fraud.',
      image: '🔐',
      tech: ['Python', 'Scikit-learn', 'Pandas', 'NumPy'],
      features: ['URL Classification', 'Feature Extraction', 'Real-time Analysis']
    }
  ];
  ...
  useEffect(() => {
    ...
    projects.forEach((project, i) => {
      ...
      card.userData = { index: i, baseAngle: angle };
      cards.push(card);
      scene.add(card);

      // Add border
      const borderGeometry = new THREE.BoxGeometry(2.05, 2.85, 0.05);
      const borderMaterial = new THREE.MeshStandardMaterial({ color: 0x111827 });
    ...
    const animate = () => {
      ...
      cards.forEach((card, i) => {
        const angle = card.userData.baseAngle + rotationRef.current;
        card.position.x = Math.cos(angle) * radius;
        card.position.z = Math.sin(angle) * radius;
        card.rotation.y = -angle + Math.PI;

        // Determine which card is at front
        const distanceToFront = Math.abs(angle % (Math.PI * 2) - Math.PI);
        if (distanceToFront < 0.3) {
          setSelectedProject(i);
        }

        // Scale effect
        const scale = 1 + (1 - Math.min(distanceToFront / Math.PI, 1)) * 0.3;
    ...
    <section id="projects" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <div className="w-20 h-1 bg-gray-900 mx-auto mb-4" />
          <p className="text-gray-600 max-w-2xl mx-auto">
            Drag to rotate • Click dots to navigate
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <span className="inline-block text-xs text-gray-900 uppercase tracking-wider font-bold bg-gray-100 px-3 py-1 rounded-full mb-3">
                    {project.category}
                  </span>
                ...
                <div className="text-6xl ml-6">{project.image}</div>
              </div>

              <div className="grid md:grid-cols-2 gap-6 mb-6">
                <div>
                  <div className="text-sm text-gray-900 uppercase tracking-wider mb-3 font-bold">Key Features</div>
                  <div className="space-y-2">
                    {project.features.map((feature, i) => (
                      <div key={i} className="text-sm text-gray-700 flex items-start gap-2">
                        <Star size={16} className="text-gray-900 flex-shrink-0 mt-0.5" />
                        <span>{feature}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <div className="text-sm text-gray-900 uppercase tracking-wider mb-3 font-bold">Tech Stack</div>
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((t, i) => (
                      <span key={i} className="px-4 py-2 bg-gray-900 text-white text-sm rounded-full font-semibold">
                        {t}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex gap-4">
                <a href="#" className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gray-900 text-white rounded-lg text-base font-semibold hover:bg-gray-800 transition-colors">
                  <Github size={20} /> View Code
                </a>
                <a href="#" className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-white border-2 border-gray-900 text-gray-900 rounded-lg text-base font-semibold hover:bg-gray-50 transition-colors">
          ...
          <div className="flex justify-center gap-3 py-6 bg-gray-50 border-t border-gray-200">
            {projects.map((_, i) => (
              <button
                key={i}
                onClick={() => rotateToProject(i)}
                className={`w-3 h-3 rounded-full transition-all ${
                  selectedProject === i ? 'bg-gray-900 w-8' : 'bg-gray-300 hover:bg-gray-400'
                }`}
              />
            ))}
          </div>
