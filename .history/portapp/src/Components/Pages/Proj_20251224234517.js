...
const Projects = () => {
  return (
    <section id="projects" className="py-24 bg-gray-900 flex justify-center">
      <div className="max-w-7xl mx-auto px-6 w-full">
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <h2 className="text-4xl md:text-5xl font-bold mb-4 flex justify-between items-center gap-3 shadow-neu-header-dark p-6 rounded-xl bg-gray-900 text-white">
            <span className="flex items-center gap-3">
              <Code2 size={40} /> Featured Projects
            </span>
            <span className="text-right text-xl md:text-2xl text-gray-400">Projects</span>
          </h2>
          <p className="text-gray-300 max-w-2xl mt-4 shadow-neu-dark p-4 rounded-xl bg-gray-900">
        ...
        <div className="flex gap-6 overflow-x-auto scrollbar-hide py-4">
          {projects.map((project, idx) => (
            <motion.div
              ...
              className="w-80 flex-shrink-0 bg-gray-900 rounded-2xl shadow-neu-dark overflow-hidden flex flex-col border border-gray-700"
            >
              {/* Project Image/Icon */}
              <div className="h-36 flex items-center justify-center text-6xl rounded-t-2xl shadow-neu-inner-dark bg-gray-900 text-white">
                {project.image}
              </div>

              {/* Project Info */}
              <div className="p-4 flex flex-col flex-grow bg-gray-900 shadow-neu-inner-dark text-white">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {project.tech.slice(0, 3).map((t, i) => (
                    <span key={i} className="px-2 py-1 bg-gray-800 text-gray-200 text-sm rounded shadow-neu-inner-dark">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="flex gap-2 mt-auto">
                  <motion.a whileHover={{ scale: 1.05 }} href="#"
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-900 text-white rounded shadow-neu-inner-dark font-medium">
                    <Github size={16} /> Code
                  </motion.a>
                  <motion.a whileHover={{ scale: 1.05 }} href="#"
                    className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-900 text-white rounded shadow-neu-inner-dark border border-gray-700 font-medium">
        ...
        {`
          ...
          .shadow-neu-inner-dark {
            box-shadow: inset 6px 6px 10px rgba(0,0,0,0.4),
                        inset -6px -6px 10px rgba(80,80,80,0.3);
          }
          .shadow-neu-header-dark {
            box-shadow: 10px 10px 20px rgba(0,0,0,0.6),
                        -10px -10px 20px rgba(80,80,80,0.4);
          }
        `}
      </style>
    </section>
